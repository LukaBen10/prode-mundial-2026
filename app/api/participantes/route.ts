...
export async function POST(req: NextRequest) {
  try {
    const { nombre_completo, nombre_usuario, mail, whatsapp, dni, password } = await req.json();
    ...
    const result = await db.execute({
      sql: 'INSERT INTO participantes (nombre, nombre_completo, nombre_usuario, mail, whatsapp, dni, password_hash, codigo) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      args: [nombre_completo.trim(), nombre_completo.trim(), nombre_usuario.trim(), mail.trim(), whatsapp.trim(), dni.trim(), hashPassword(password), codigo],
    });
